...
class App{
  constructor(canvas, overlay) {
    ...
    this.mouse = {x: null, y: null, down: false, pressed: false};
  ...
  registerEventHandlers() {
    document.onkeydown = (event) => {
      ...
      this.keysPressedDown[keyNames[event.keyCode]] = true;
    };
    document.onkeyup = (event) => {
      //jshint unused:false
      this.keysPressed[keyNames[event.keyCode]] = false;    
      this.keysPressedDown[keyNames[event.keyCode]] = false;
    };
  ...
  update() {
    ...
    if (pendingResourceNames.length === 0) {
      ...
      this.scene.update(this.gl, this.keysPressed, this.keysPressedDown, this.mouse);
